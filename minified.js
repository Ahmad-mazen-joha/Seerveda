...
window.saveData = async (sectionKey) => {
    ...
    const docRef = doc(db, "websiteContent", "main");
    try {
        await updateDoc(docRef, { [sectionKey]: dataToSave });
...
if (submitConditionV2Btn && conditionV2FormEl) {
    submitConditionV2Btn.addEventListener("click", () => {
        ...
        const imageRows = imagesContainer ? Array.from(imagesContainer.querySelectorAll(".condV2ImageRow")) : [];
        ...
        const newItem = { id: generateId(10), name, date, text, images };
...
submitConditionButton.addEventListener("click", () => {
    ...
    const newCondition = { conditionName: condName.trim(), conditionLink: condLink.trim() };
...
const renderManagement = () => {
    ...
    mangementData.forEach((member) => {
        ...
        memberItem.textContent = `${member.title} ${member.name} (${member.tag || "management member"})`;
...
saveMemberButton.addEventListener("click", () => {
    ...
    if (!memberName || !memberEmail || !memberPhone || !memberTitle || !memberImage || !memberText) {
...
function gatherEventFormFilters() {
    const selected = Array.from(eventForm.querySelectorAll(".eventFilterCheckbox:checked")).map((checkbox) =>
        String(checkbox.value).trim()
    );
    const customFilterValues = Array.from(eventForm.querySelectorAll(".eventCustomFilterInput"))
        .map((input) => String(input.value || "").trim())
        .filter(Boolean);
...
function clearEventFormFilters() {
    const container = eventForm.querySelector(".eventFiltersContainer");
    if (container) {
        container.querySelectorAll(".eventFilterCheckbox").forEach((cb) => (cb.checked = !1));
    }
    const customFilterContainer = eventForm.querySelector(".eventCustomFiltersContainer");
    if (customFilterContainer) {
        customFilterContainer.innerHTML = "";
...
if (eventForm) {
    eventForm.addEventListener("input", (event) => {
        if (event.target.classList.contains("eventDesc") || event.target.classList.contains("eventShortDesc")) {
...
saveNewEventButton.addEventListener("click", () => {
    ...
    const newEvent = { img1: normalizeCloudinaryUrl(img1), img2: normalizeCloudinaryUrl(img2) };
...
const renderProducts = () => {
    ...
    productsData.forEach((item) => {
        ...
        (item.conditions || []).forEach((condition) => {
            ...
            deleteConditionButton.setAttribute("data-cond-name", condition.conditionName || "");
            deleteConditionButton.setAttribute("data-product-id", item.id ? item.id : 1);
            deleteConditionButton.addEventListener("click", () => {
                ...
                const product = productsData.find((p) => p.id == productId);
                if (product) {
                    product.conditions = (product.conditions || []).filter((c) => c.conditionName !== condName);
                    renderProducts();
                }
            });
        ...
        (item.conditionsV2 || []).forEach((cv2) => {
            ...
            li.textContent = `${cv2.name || ""} ${cv2.date ? "(" + cv2.date + ")" : ""}`;
            const del = document.createElement("button");
            del.type = "button";
            del.textContent = "Delete V2";
            del.addEventListener("click", () => {
                item.conditionsV2 = (item.conditionsV2 || []).filter((c) => c.id !== cv2.id);
                renderProducts();
            });
            li.appendChild(del);
        ...
        showConditionsButton.addEventListener("click", () => {
            ...
            showConditionsButton.textContent = isHidden ? "Hide Medical clinical cases" : "Show Medical clinical cases";
        });
        ...
        conditionsV2Button.addEventListener("click", () => {
            ...
            const images = conditionV2FormEl.querySelector(".condV2Images");
            if (images && images.children.length === 0) images.appendChild(createCondV2ImageRow());
        });
    ...
    let newdata = onSnapshot(
        ...
        (doc) => {
            if (doc.exists()) {
                ...
                const managementFilterBtns = document.querySelectorAll(".management-filter-btn");
                if (managementFilterBtns && managementFilterBtns.length) {
                    managementFilterBtns.forEach((btn) => {
                        btn.addEventListener("click", () => {
                            ...
                            const visibleMembers =
                                filter === "all"
                                    ? members
                                    : members.filter((m) => (m.tag || "management member") === filter);
                            const slidesHTML = visibleMembers.map((m) => {
                                const tagAttr = m.tag ? `data-tag="${m.tag}"` : `data-tag="management member"`;
                  ...
                  <div class="card swiper-slide" ${tagAttr}>
                    ...
                    <div class="card__content">
                      ...
                      <button class="card__btn">VIEW MORE</button>
                            ...
                            if (
                                ...
                                if (typeof resetSwiperSlideStyles === "function") resetSwiperSlideStyles(newSlides);
                            } else {
                    ...
                    const allBtn = document.querySelector('.management-filter-btn[data-filter="all"]');
                    if (allBtn) allBtn.classList.add("active");
                }
                let cards = [];
                async function generateproductsCardsDependsOnRespnse(contType, contType2) {
                    ...
                    function getClassTokensFromType(s) {
                        ...
                        if (parts.length === 2) return [`${capitalizeWord(parts[0])}-${capitalizeWord(parts[1])}`];
                ...
                const outerCondImages1 = document.querySelectorAll(".outerCondImages1 img");
                const outerCondImages2 = document.querySelectorAll(".outerCondImages2 img");
                ...
                let bringProductData = (product, button) => {
                    for (val of data.products) {
                        if (val.title === product) {
                            ...
                            moreProductImage.src = normalizeCloudinaryUrl(choosenProduct.prodImage);
                                    ...
                                    condsV2.forEach((c) => {
                                        ...
                                        if (Array.isArray(c.images) && c.images.length) {
                                            ...
                                            c.images.forEach((img) => {
                                                ...
                                                im.loading = "lazy";
                                                im.decoding = "async";
                                                im.alt = img.alt || `${choosenProduct.title} - ${c.name || ""}`;
                                                fig.appendChild(im);
                ...
                let addAncherTobutton = document.querySelectorAll(".eventCard div:nth-child(2) button");
            ...
            `;
                    ...
                    closeButton?.addEventListener("click", closeModal, { once: !0 });
                ...
                let getEventImages = (event) => {
                    ...
                    return [event.img1, event.img2, event.imageUrl, event.imageurl].filter(Boolean);
                };
                ...
                let updateEventCarouselView = () => {
                    ...
                    if (afterbutton) {
                        afterbutton.classList.toggle("disabled", eventPageIndex >= eventPageGroups.length - 1);
                    }
                };
                let createEventCard = (currentEvent, cardIndex) => {
                    ...
                    eventText.appendChild(document.createTextNode(currentEvent?.["short-desc"] || ""));
                    ...
                    eventButton.setAttribute("data-set-num", cardIndex);
                    ...
                    eventButton.addEventListener("click", (e) => {
                        ...
                        e.target.getAttribute("data-set-num") == 0
                            ? ""
                            : document
                                  .querySelector(".left-sid-cont #desc")
                ...
                let makeCard = (arr) => {
                    ...
                    for (let eventCounter = 0; eventCounter < arr.length; eventCounter += 3) {
                        ...
                        pageEvents.forEach((currentEvent, index) => {
                            pageDiv.appendChild(createEventCard(currentEvent, eventCounter + index));
                        });
                ...
                let generateFilterd = (arr) => {
                    ...
                    for (let i = 0; i < arr.length; i++) {
                        ...
                        const matchesFilter = typeCond === "all" || eventFilters.includes(typeCond);
